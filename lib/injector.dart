...
class MethodBodyInjector extends RecursiveAstVisitor {
  ...
  visitBlockFunctionBody(BlockFunctionBody node) {
    ...
    if (decl.returnType?.name?.name=="void") {
      ...
      edit.edit(node.offset, node.offset, "{ ${AopWrappers.AOP_WRAPPER_METHOD_NAME}($newInvokationContextText,() ");
    } else {
      edit.edit(node.end, node.end, ");}");
      edit.edit(node.offset, node.offset, "{ return ${AopWrappers.AOP_WRAPPER_METHOD_NAME}($newInvokationContextText,() ");
...
class AopMixinInjector extends RecursiveAstVisitor {
  ...
  visitClassDeclaration(ClassDeclaration classDecl) {
    ...
    if (edit.hasEdits) {
      edit.edit(classDecl.name.end,classDecl.name.end," extends Object with AopWrappers");
    }
...
class MethodInterceptorPointcut extends PointcutInterceptor with RecursiveAstVisitor {
  String id;
  ...
  visitMethodDeclaration(MethodDeclaration node) {
    logger.fine("Method : ${node.name.toString()} : ${node.offset} - ${node.end}");
    if (matcher.matches(node)) {
      // Recurr on body and inject callbacks to Aop
      String newInvokationContextText = _createInvokationContextInitializerCode(node);
  ...
  String _createInvokationContextInitializerCode(MethodDeclaration node) {
    StringBuffer sb = new StringBuffer("new InvokationContext('$id','${node.name}',[");
    String sep="";
    node.parameters.parameters.where((FormalParameter fp) => fp.kind!=ParameterKind.NAMED).forEach((FormalParameter fp) {
      sb.write(sep);
...
class InvokationContext {
  ...
  InvokationContext(this.pointcutId,this.methodName,this.positionalParameters,this.namedParameters);
